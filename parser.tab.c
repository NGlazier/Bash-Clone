...
{
       0,    62,    62,    63,    66,    67,    71,    70,    75,    76,
      80,    87,   104,   113,   212,   218,   224,   238,   244,   249,
     262,   273,   284,   295
};
#endif
...
static const char *const yytname[] =
{
  ...
  "OUTPUT_APPEND", "ERROR", "ERROR_APPEND", "WORD", "$accept", "input",
  "lines", "oneline", "$@1", "eoln", "line", "cmd", "args", "arg",
  "optredirs", "optredir", YY_NULLPTR
};
#endif
...
{
       1,     0,    -8,    -5,     8,    -8,     6,    -8,    -8,     9,
      -8,    -8,    12,    -5,    -8,    -8,     0,     2,     4,    13,
      14,    15,    16,    -8,    12,    -8,    -8,    -8,    -8,    -8,
      -8,    -8,    -8,    -8
};
...
{
       0,     0,    10,    15,     0,     2,     0,     8,     6,    12,
       9,    16,    18,    15,     1,     5,     0,     0,     0,     0,
       0,     0,     0,    13,    18,    14,     7,    11,    19,    20,
      22,    21,    23,    17
};
...
{
      10,    -3,     1,     2,     2,    11,    -4,     1,    14,     2,
      27,     3,     3,    17,    28,    26,     3,    18,    19,    20,
      21,    22,    33,    29,    30,    31,    32,    15,     0,     0,
      25
};

static const yytype_int8 yycheck[] =
{
       1,     0,     1,     3,     3,    10,     0,     1,     0,     3,
      17,    10,    10,     4,    10,    16,    10,     5,     6,     7,
       8,     9,    24,    10,    10,    10,    10,     6,    -1,    -1,
      13
};
...
#line 88 "parser.y"
			...
			if((yyvsp[-2].pcmd) -> outfile != NULL){
				printf("ERROR multiple output redirections dectected.\n");
				synerrors++;
			}
			(yyvsp[-2].pcmd) -> outfile = "PIPE";
			if((yyvsp[0].pcmd) -> infile != NULL){
				printf("ERROR multiple output redirections dectected.\n");
				synerrors++;
			}
		   (yyvsp[0].pcmd) -> infile = "PIPE";
		   (yyvsp[-2].pcmd) -> nextpipe = (yyvsp[0].pcmd);
...
#line 114 "parser.y"
			...
			pcd -> command = (yyvsp[-2].string);
			pcd -> argv[0] = (yyvsp[-2].string);
			int arg_count = 1;
			
			if((yyvsp[-2].string)[0] == '/' || ((yyvsp[-2].string)[0] == '.' && (yyvsp[-2].string)[1] == '/') || ((yyvsp[-2].string)[0] == '.' && (yyvsp[-2].string)[1] == '.' && (yyvsp[-2].string)[2] == '/')){
				char *p = strrchr((yyvsp[-2].string), '/');
				if (p && *(p + 1)){
					pcd -> argv[0] = p+1;
				}
			}
			...
			while((yyvsp[-1].pargs) != NULL){
				
				pcd -> argv[arg_count] = (yyvsp[-1].pargs) -> arg; 
				(yyvsp[-1].pargs) = (yyvsp[-1].pargs) -> next;
			...
			while((yyvsp[0].predir) != NULL){
				if((yyvsp[0].predir) -> dir == 1){ //1 to represent input
					...
					pcd -> infile = (yyvsp[0].predir) -> fname;
				}
				if((yyvsp[0].predir) -> dir == 2){ //2 to represent output
					
					if(pcd -> outfile != NULL){
						printf("ERROR multiple output redirections dectected.\n");
						synerrors++;
					}
					pcd -> outfile = (yyvsp[0].predir) -> fname;
				}
				if((yyvsp[0].predir) -> dir == 3){ //3 to represent error
					if(pcd -> outfile != NULL){
						printf("WARNING file redirection arguments should be of the order < i 2> e > o\n");
					}
					if(pcd -> errfile != NULL){
					...
					pcd -> errfile = (yyvsp[0].predir) -> fname;
				}
				if((yyvsp[0].predir) -> dir == 4){ //4 to represent output_append
				...
				if((yyvsp[0].predir) -> dir == 5){ //5 to represent error_append
				if(pcd -> outfile != NULL){
						printf("WARNING file redirection arguments should be of the order < i 2> e > o\n");
					}
				if(pcd -> errfile != NULL){
				...
				(yyvsp[0].predir) = (yyvsp[0].predir) -> next;
			}
